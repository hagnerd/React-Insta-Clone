...
export default class CommentSection extends React.Component {
  ...
  deleteComment = (id) => {
    this.setState(prevState => ({
      comments: prevState.comments.filter(comment => !(comment.id === id && comment.username === this.props.currentlyLoggedInUser)),
    }))
  }
      ...
      <ul>
        {this.state.comments.map(comment => 
          ...
          {this.props.currentlyLoggedInUser === comment.username && (
            <button className="bg-red-400 border border-red-500 rounded-sm px-3 py-2 my-1 hover:bg-red-500 hover:border-red-600" onClick={() => this.deleteComment(comment.id)}>delete comment</button>
          )}
      </>
      )
        }
